...
```admonish quote title=""
{{exec}}{exe:NPaltdef}[练习15.1（$\mathbf{NP}$的另一定义）]
...
```admonish quote title=""
{{remc}}{rem:NPnotcomplement}[备注15.2（$\mathbf{NP}$（不一定）在补运算下封闭）]
...
```admonish quote title=""
{{exec}}{exe:3SATinNP}[例15.3（$3\mathit{SAT} \in \mathbf{NP}$）]
...
我们可以用一个长度为 $k$ 的字符串 $w$ 来表示对 $k$ 个变量的赋值。以下算法检查给定的 $w$ 是否满足公式 $\varphi$：

```admonish quote title=""
{{algc}}{alg:3SATverifier}[算法15.4（$3\mathit{SAT}$ 的验证器）]
...
```admonish quote title=""
{{exec}}{exe:Verifying}[练习15.2（验证不比解决更难）]
证明$\mathbf{P} \subseteq \mathbf{NP}$
```

```admonish quote title=""
{{remc}}{rem:NPlabel}[备注15.5（$\mathbf{NP}$ 不代表非多项式时间！）]
...
```admonish quote title=""
{{exec}}{exe:NPEXP}[练习15.3 （$\mathbf{NP}$属于$\mathbf{EXP}$）]
...
```admonish quote title=""
{{exec}}{exe:reductionNP}[练习15.4（归约与$\mathbf{NP}$）]
...
```admonish quote title=""
{{thmc}}{thm:CookLevin}[定理15.6（Cook-Levin定理）]
...
```admonish quote title=""
{{defc}}{def:NPhardNPC}[定义15.7（$\mathbf{NP}$ 困难和$\mathbf{NP}$ 完全）]
