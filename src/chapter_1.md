...
在本章中, 我们将会介绍一些将在本书中用到的数学概念. 这些概念一般会在"计算机科学中的数学"或"离散数学"等课程或课本中讲解. 有关这些主题的几份可在线免费获取优秀资源, 请参阅"参考书目"部分([第1.9节](#notesmathchap)). 
...
- 如果你已经学习过"离散数学"、"计算机科学中的数学"或任何类似课程, 则无需阅读整章内容, 只需要快速地阅读[第1.2节](#secmathoverview)来了解我们会用到什么数学工具与[第1.7节](#notationsec)来了解本书所用符号, 便可跳转至后续章节. 或者, 你也可以放松心情通读本章, 既熟悉本书所用的符号体系, 顺便品味(或忍受)笔者融于字里行间的哲学思考与幽默尝试. 
- 若相关基础较为薄弱, 可以参考[第1.9节](#notesmathchap)中提供的学习资源. 本章虽然已经涵盖了所有所需知识点, 但系统性地学习相关知识点可能对你更有帮助. 数学学习重在实践, 通过独立完成练习才能真正掌握这些内容. 
...
我们将使用的主要数学概念如下所示. 此处仅列出这些概念, 其具体定义将在本章后续部分给出. 若您已熟悉所有这些内容, 可以直接跳至[第1.7节](#notationsec)查看我们使用的完整符号列表. 
...
一个**集合**是一些对象的无序容器. 例如, $S=\{2,4,7\}$表示$S$指代一个包含数字$2$、$4$、$7$的集合(我们使用$2\in S$来表示$2$是$S$中的一个元素. )注意集合$\{2,4,7\}$与$\{7,4,2\}$是相同的, 因为它们拥有相同的元素. 同时, 一个集合要么包含一个元素, 要么不包含一个元素, 不存在"包含两次"的概念, 因此我们甚至可以将同一个集合$S$写作$\{2,2,4,7\}$(尽管这样写有些奇怪). 有限集合的 **基数** (cardinality), 即一个集合包含的元素的数量, 记作$|S|$(基数亦可以定义在 **无限** 集上, 见[第1.9节](#notesmathchap)的参考资料). 因此在上例中$|S|=3$. 若集合$S$的元素都是集合$T$的元素, 则称$S$为$T$的一个**子集**, 记作$S\subseteq T$(我们亦可以称$T$为$S$的一个**超集**. )比如, $\{2,7\}\subseteq \{2,4,7\}$. 不包含任何元素的集合称作**空集**, 写作$\emptyset$. 如果$A$是$B$的一个子集且$A$不等于$B$, 则我们称$A$为$B$的一个**真子集**, 记作$A\subsetneq B$. 
...
*图*在计算机科学及众多其他领域中无处不在. 图可以用于建模非常多的数据类型, 包括但不限于社交网络、调度约束、道路网络、深度神经网络、基因相互作用、观测值之间的相关性. 几种图的正式定义将在下面给出, 但如果你没有在先前的课程中了解过图, 我强烈建议你从[第1.9节](#notesmathchap)中的资料中详细了解它们. 
...
下面是一些关于无向图的基本事实. 我们将为它们给出一些非正式的论证, 但完整证明作为练习留待读者自行完成(完整证明可以在[第1.9节](#notesmathchap)中的诸多资源中找到). 
...
```admonish info
{{rem}}{rem:comingupwithproofs}[寻找证明的方法]
...
```admonish info
{{rem}}{rem:bigonotime}[大$O$表示法的其他应用场景(可选)]
...
**数学归纳法证明**: 我们将在下面的[第1.6.1节](#inductionsec)中讨论数学归纳法并给出示例. 我们可以将这类证明视为上述方法的变体, 其中我们有无穷多个中间结论$X_0,X_1,X_2,\dots,X_k$, 并证明$X_0$成立, 且$X_0$蕴含$X_1$, $X_0\wedge X_1$蕴含$X_2$, 依此类推. 卡内基梅隆大学15-251课程的网站提供了一份[有用的讲义](http://www.cs.cmu.edu/~arielpro/15251f17/notes/induction-pitfalls.pdf), 介绍了使用数学归纳法时可能遇到的常见陷阱. 
