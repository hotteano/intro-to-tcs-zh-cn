...
本章讨论的两个 "核心思想" 是: {{ref:bigidea:representtuples}} -- 我们可以通过组合简单对象的表示来表示更复杂的对象; 以及 {{ref:bigidea:functionprogram}} -- 区分 函数 的 "what" 与 程序 的 "how" 至关重要. 后者将是本书中反复提到的一个主题.
```
...
[第2.2.1节](#repnegativeintegerssec) 中使用特定的"符号位"来表示整数的方法被称为 **有符号数表示法 (Signed Magnitude Representation)**, 曾在一些早期计算机中使用.  
...
{{example}}{example:represnumberbypairs}[将一个有理数表示为字符串]
考虑有理数 $r=-5/8$.  
...
{{bigidea}}{bigidea:representtuples}
...
利用函数的基本性质(见 [第1.4.3节](#functionsec)), 一个集合可数当且仅当存在从 $S$ 到 $\{0,1\}^*$ 的一一函数.  
...
~~~admonish example title=""
{{example}}{example:braille}[Braille 编码(盲文)]
...
~~~admonish example title=""
{{example}}{example:Crepresentation}[C语言中对象的表示(选读)]
...
{{bigidea}}{bigidea:functionprogram}
**函数** 与 **程序** 并不相同.  
程序是用来 **计算** 一个函数的.
```
