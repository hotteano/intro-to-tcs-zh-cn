...
在任何计算理论课程中的某个时刻, 教师和学生都需要进行*那次*谈话. 也就是说, 我们需要讨论描述算法时的*抽象层次*. 在算法课程中, 通常用英语描述算法{{footnote: 译者注: 在本翻译版中会使用中文}}, 假设读者能够"填充细节", 并在需要时能够将此类算法转化为实现. 例如, {{ref:algc:a81}}是广度优先搜索算法的高级描述. 
