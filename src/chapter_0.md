...
一般来说, 算法的分析不仅会包含对算法的**正确性**分析, 还会包含对算法**高效性**的分析. 也就是说, 我们不仅想证明算法完成了预计的任务, 而且会在规定的次数内完成. 比如说, {{ref:alg:gradeschool}}使用了$O(n^2)$次操作完成了对$n$位数字的乘法, 而{{ref:alg:karatsuba}}(在下一节中介绍)使用了$O(n^{1.6})$次操作完成了同样的操作(我们会在第1.4.8节中定义大$O$表示法)
```
