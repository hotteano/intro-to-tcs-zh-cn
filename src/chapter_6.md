...
```admonish question title="一般函数的布尔化"
{{exec}}{exec:booleanize}[已解答练习 6.1]
...
```admonish question title="识别$(010)^*的DFA$"
{{exec}}{exec:DFAzeroone}[已解答练习 6.2]
...
```admonish example title="样例6.9 一个正则函数"
...
```admonish question title="匹配空串"
{{exec}}{exec:emptymatchex}[已解答练习 6.3]
...
剩下的唯一任务就是分析其 **运行时间** 。
需要注意的是，{{tref:alg:regexpmatchlinearalg}}在归纳奠基$x=\texttt{""}$时使用{{tref:proc:emptymatchex}}中的$\text{\textsf{MatchEmpty}}$过程。
...
```admonish proof collapsible=true, title = "证明"
既然{{tref:thm:DFAforREGthm}}已经证明了“仅当”方向，现在只需要证明“当”方向。
...
现在将要证明$F_{v,w}$对每个$v,w$都正则。这将证明该定理。因为根据{{tref:def:DFAdef}}，$F(x)$等于对所有$F_{0,w}(x)$取或，其中$w\in \mathcal{S}$。
...
因为$a \wedge b = \overline{\overline{a} \vee \overline{b}}$，{{tref:lem:regcomplementlem}}表明正则函数族在与操作下也同样封闭。进一步说，因为或，非，与是通用的基础运算，这个集合在与非，异或，和其它有限函数的运算下也封闭。
这就是说，我们有如下推论
...
```admonish proof collapsible=true, title = "证明"
这是正则函数在或运算和非运算（因此也有与运算）下的封闭性，与{{tref:thm:circuit-univ-thm}}(注:定理4.13)——其声明每个$f$都可以被一个布尔电路计算（其只不过是与、或、非运算的结合）——结合的直接结果。$\square$
```
...
```admonish quote title="图 6.9"
![pumpinglemma](./images/chapter6/pumpinglemma.png)
...
```admonish proof collapsible=true, title = "证明思路"
证明思路如下。令$n_0$为表达式$e$中使用的字母数的两倍，串$w$满足$|w|>n_0$且$\Phi_{e}(w)=1$，则串$w$存在的唯一方法是$e$中含有操作$*$(即，闭包操作)，且$w$有一个非空子串$y$被$(e')*$匹配，其中$e'$为$e$的子串。（译者注：此处应为作者笔误，正确语句应当如下：且$w$有一个非空子串$y$被$e'$匹配，其中$(e')*$为$e$的子串。）我们可以重复$y$任意多次，而所得的串仍然被匹配。又见[图 6.9](#pumpinglemma)
```
...
```admonish question title="回文非正则"
{{exec}}{exec:palindromenotreg}[已解答练习 6.4]
...
此处采用泵引理。
为了使用反证法，假设有一个正则表达式$e$计算$PAL$，令$n_0$为泵引理（{{tref:thm:pumping}}）中的数。考虑串$w = 0^{n_0};0^{n_0}$。因为全部由零组成的串的反转仍为全部由零组成的串，所以$PAL(w)=1$。
...
另一个基于泵引理的证明见[图 6.10](#pumpinglemmaproof)，这是一个关于函数$F:\{0,1\}^* \rightarrow \{0,1\}$非正规性证明的漫画，其中$F(x)=1$当且仅当存在$n\in \N$使得$x=0^n1^n$（即，$x$为一个连续零串拼接上一个同等长度的连续一串）。
...
```admonish quote title=""
{{thm}}{thm:regemptynessthm}[定理6.23] 正则语言的空性可计算
...
通过{{tref:thm:regemptynessthm}}，我们可以得到判定两个正则表达式是否 **等价** 的算法。这意味着它们计算相同的函数。

```admonish quote title=""
{{thm}}{thm:regequivalencethm}[定理 6.24] 正则表达式的等价性可计算
...
```admonish proof collapsible=true,title="证明"
我们从{{tref:thm:regemptynessthm}}中证明{{tref:thm:regequivalencethm}}。（这两个定理实际上是等价的：我们很容易从{{tref:thm:regequivalencethm}}中证明{{tref:thm:regemptynessthm}}，因为测试表达式空性和判定其与$\emptyset$的等价性是一样的。）
...
特别地，根据{{tref:thm:regcomplementlem}}，正则函数在否运算下封闭。这意味着对每个正则表达式$e$，均有表达式$\overline{e}$使得对所有$x\in \{0,1\}^*$均有$\Phi_{\overline{e}}(x) = 1 - \Phi_{e}(x)$。
...
给出了这两个变换，可以发现对所有的正则表达式$e$与$e'$，都可以找到一个表达式$e''$满足{{tref:eq:63}}，使得$e''$为空当且仅当$e$与$e'$等价。$\square$

```
```admonish note title="章节回顾"
...
```admonish question title="习题6.1 正则函数的闭性质"
{{proc}}{proc:closureregex}[习题6.1]
...
```admonish question title="习题6.2"
{{proc}}{proc:regularno}[习题6.2]
...
```admonish question title="习题6.3 非正则性"
{{proc}}{proc:nonregex}[习题6.3]
...
我们对于{{tref:thm:reglintimethm}}的证明与[Myhill-Nerode定理](https://goo.gl/mnKVMP)联系紧密。Myhill-Nerode定理的一个方向可以被陈述为：如果$e$是一个正则表达式，则存在最多有限个串$z_0,\ldots,z_{k-1}$，使得对每个$0 \leq i\neq j < k$，有$\Phi_{e[z_i]} \neq \Phi_{e[z_j]}$